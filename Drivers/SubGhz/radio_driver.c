...
static const FskBandwidth_t FskBandwidths[] = {
    ...
    {467000, 0x09}, {500000, 0x00}, // Invalid Bandwidth
};
...
RadioOperatingModes_t SUBGRF_GetOperatingMode(void) { return OperatingMode; }
...
void SUBGRF_SetRxBoosted(uint32_t timeout) {
  ...
  SUBGRF_WriteRegister(REG_RX_GAIN,
                       0x97); // max LNA gain, increase current by ~2mA for
                              // around ~3dB in sensitivity
...
RadioPacketTypes_t SUBGRF_GetPacketType(void) { return PacketType; }
...
void SUBGRF_SetTxParams(uint8_t paSelect, int8_t power,
  ...
  if (paSelect == RFO_LP) {
    ...
    SUBGRF_WriteRegister(REG_OCP,
                         0x18); /* current max is 80 mA for the whole device*/
  } else                        /* rfo_hp*/
  {
    /* WORKAROUND - Better Resistance of the RFO High Power Tx to Antenna
     * Mismatch, see STM32WL Erratasheet*/
    SUBGRF_WriteRegister(REG_TX_CLAMP,
                         SUBGRF_ReadRegister(REG_TX_CLAMP) | (0x0F << 1));
    /* WORKAROUND END*/
    ...
    SUBGRF_WriteRegister(REG_OCP,
                         0x38); /*current max 160mA for the whole device*/
  }
  buf[0] = power;
...
void SUBGRF_SetModulationParams(ModulationParams_t *modulationParams) {
  ...
  case PACKET_TYPE_GFSK:
    n = 8;
    tempVal =
        (uint32_t)((32 * XTAL_FREQ) / modulationParams->Params.Gfsk.BitRate);
  ...
  case PACKET_TYPE_BPSK:
    n = 4;
    tempVal =
        (uint32_t)((32 * XTAL_FREQ) / modulationParams->Params.Bpsk.BitRate);
    buf[0] = (tempVal >> 16) & 0xFF;
    buf[1] = (tempVal >> 8) & 0xFF;
    buf[2] = tempVal & 0xFF;
...
void SUBGRF_SetPacketParams(PacketParams_t *packetParams) {
  ...
  case PACKET_TYPE_GFSK:
    ...
    buf[3] = (packetParams->Params.Gfsk
                  .SyncWordLength /*<< 3*/); // convert from byte to bit
...
void SUBGRF_GetRxBufferStatus(uint8_t *payloadLength,
...
uint32_t SUBGRF_GetRadioWakeUpTime(void) { return RF_WAKEUP_TIME; }
...
void HAL_SUBGHZ_CADStatusCallback(SUBGHZ_HandleTypeDef *hsubghz,
